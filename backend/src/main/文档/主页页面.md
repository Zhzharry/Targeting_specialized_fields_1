...
-**说明**：根据当前查看的房源ID，返回相似房源推荐列表。优先使用已计算的相似度，不足时实时计算补足。
-**请求参数**：
- `propertyId`（必填，Long）：当前查看的房源ID
- `userId`（选填，Long）：用户ID，用于过滤已看房源
- `limit`（选填，Integer）：返回数量，默认5
-**请求示例**：GET /api/profile/recommend/similar?propertyId=12345&limit=5
-**成功响应示例**：
